...
export default class DashboardScreen extends Component {
  ...
  _retriveData = async () => {
    await Utils.getData(
      ...
      (value = data => {
        ...
        this.setState({
          audioEnable: parseInt(data.value) == NaN ? 0 : parseInt(data.value),
        });
      }),
    );
    await Utils.getData(
  ...
  _saveUserLocation(location) {
    Utils.ApiPost(
      `${R.constants.Api.saveUserLocation}${this.state.user_id}&geo_latitude=${location.latitude}&geo_longitude=${location.longitude}`,
      (response = data => {
        if (data.res == 200) {
  ...
  _getTotalProfile = async () => {
    await Utils.ApiPost(
      `${R.constants.Api.profileList}${this.state.user_id}`,
      (response = data => {
        //profileList Api
        if (data.res == 200) {
          if (data.data.res_status == 'success') {
  ...
  _profileDetailWebSearvice() {
    Utils.ApiPost(
      `${R.constants.Api.profileDetails}${this.state.user_id}&limit_offset=${this.state.currentOffset}`,
      (response = data => {
        if (data.res == 200) {
          if (data.data.res_status == 'success') {
  ...
  _updateUserLike() {
    ...
    Utils.ApiPost(
      `${R.constants.Api.likeUnlike}${this.state.entries.profile_id}&like_by=${
        this.state.user_id
      }&action=${this.state.entries.ilike_profile == 0 ? 1 : 0}`,
      (response = data => {
        if (data.res == 200) {
          if (data.data.res_status == 'success') {
  ...
  pagination(item) {
    return (
      <View
        style={{position: 'absolute', zIndex: 10, top: 5, right: 0, left: 0}}>
        <Pagination
          ...
          inactiveDotScale={1}
        />
        <LinearGradient colors={['#232323', '#ffffff00']}>
          <View
            style={{flexDirection: 'row', margin: 10, alignItems: 'center',justifyContent:"space-between"}}>
            <TouchableOpacity
              onPress={() => {
                this.stopAudio(),
                  this.props.onProfileClick(this.state.entries.profile_id);
              }}
              style={{flexDirection: 'row', justifyContent: 'center'}}>
              <View style={{}}>
                <Image
                  source={{
                    uri: this.state.entries.profile_images[0].profile_img,
                  }}
                  style={{
                    height: 45,
                    width: 45,
                ...
                <View
                  style={{
                    backgroundColor:
                      this.state.entries.profile_online == 'online'
                        ? '#24ff02'
                        : 'grey',
                    height: 15,
                    width: 15,
              ...
              <View style={{paddingHorizontal: 10}}>
                ...
                <Text style={{fontSize: 14, color: 'white'}}>
                  {this.state.entries.current_age}Yrs,{' '}
                  {this.state.entries.distance_km}kms
                </Text>
              </View>
            </TouchableOpacity>
    ...
    return (
      <Modal
        ...
        {this.state.modalContentValue == 0
          ? this.likeModalContent()
          : this.state.modalContentValue == 1
          ? this.dislikeModalContent()
          : null}
      </Modal>
    );
  }
    ...
    return (
      <View
        ...
        <View
          ...
          <Image style={{height: 40, width: 40}} source={R.images.cancel} />
        </View>
      </View>
    );
  }
    ...
    return (
      <View
        ...
        <View
          ...
          <Image style={{height: 40, width: 43}} source={R.images.heart} />
        </View>
      </View>
    );
  }
  ...
  btnSpeakerPress() {
    ...
    this.setState({audioEnable: this.state.audioEnable == 0 ? 1 : 0}, () =>
      Utils.storeData('sound', this.state.audioEnable),
    );
  }

  playAudio() {
    console.log(this.state.audioUri);
    profileAudio = new Sound(this.state.audioUri, '', (error, sound) => {
      if (error) {
      ...
      (response = data => {
        ...
        if (data.res == 200) {
          if (data.data.res_status == 'success') {
            this.setState({
              nR: false,
              entriesModal: data.data.gift_list,
              giftsLoading: false,
            });
          }
  ...
  navigateGiftShop = () => {
    ...
    // this.props.route.params.backFromChatScreen("navGift","");
  ...
  sendGift = (gift_id, payment_id) => {
    Utils.ApiPost(
      `${R.constants.Api.sendGift}${this.state.user_id}&profile_id=${this.state.entries.profile_id}&gift_id=${gift_id}&payment_id=${payment_id}`,
      (response = data => {
              ...
              <View style={{backgroundColor: '#fff'}}>
                    ...
                    renderItem={({item}) => {
                      return (
                        <View
                          ...
                          <TouchableOpacity
                            onPress={() =>
                              this.sendGift(item.gift_id, item.payment_id)
                            }
                            style={{
                ...
                {this.state.nR && (
                  <View
                    ...
                    <Text style={{color: '#fff'}}>No Gifts</Text>
                  </View>
                )}
              </View>
            )}
      ...
      <View style={{flex: 1}}>
        ...
        {!this.state.isLoading && this.state.entries !== '' && (
          <View style={{flex: 1}}>
            ...
            <View
              style={[
                this.state.entries.myself_usehand == 20
                  ? stylesLocal.buttonPlacementRight
                  : stylesLocal.buttonPlacementLeft,
                {
                  position: 'absolute',
                  top: 0,
                  bottom: 0,
              ...
              <View
                ...
                <TouchableOpacity
                  onPress={() =>
                    this.props.onCall(this.state.entries, 'outgoing')
                  }
                  style={{
                    backgroundColor: '#ffffff',
                    padding: 13,
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    this.setState({giftsLoading: true}, () => {
                      this.AlertBoxVisibilityModal(), this.getgiftList();
                    })
                  }>
              ...
              <View
                ...
                <TouchableOpacity
                  ...
                  onPress={() => this.btnSpeakerPress()}>
                  <Image
                    style={{height: 18, width: 18}}
                    source={
                      this.state.audioEnable == 0
                        ? R.images.mute
                        : R.images.sound
                    }
                  />
            ...
            <View
              ...
              <TouchableOpacity
                ...
                <Image
                  ...
                  source={
                    this.state.entries.ilike_profile == 1
                      ? R.images.heart
                      : R.images.heart_grey
                  }
                />

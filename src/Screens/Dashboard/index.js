...
export default class Dashboard extends React.Component {
  constructor(props) {
    ...
    DeviceEventEmitter.addListener('decline_call', res => {
      // console.log('decline_call======> event====>', res.data); // kapoor
    ...
    DeviceEventEmitter.addListener('recieve', res => {
      console.log('receive in dashboard index.js=================', res);
      this.setState({callData: res.data}, () => {
        this.AcceptCall();
      });
    });
  ...
  callStatus(res) {
    Utils.ApiPost(
      ...
      (response = data => {
        console.log('callStatus==DashBoard/index.js==>', data);
        if (data.res == 200) {
  ...
  openHelpOption(item) {
    ...
    if (item.id === '2') {
      this.props.navigation.navigate('WebViewScreen', {
        url: 'https://dadio.in/privacy-policy.html',
      });
    } else {
            ...
            <TouchableOpacity
              onPress={() => {
                ...
                if (this.state.isRecording) {
                  this.setState({isRecording: false, timer: 10}, () => {
  ...
  AcceptCall() {
    console.log('Accept in dashboard index.js ===============');
    this.callAction('answer');
    console.log('accept in dashboard index.js 1111 =================');
    InCallManager.stopRingtone();
    // InCallManager.start();
  }

  async callAction(type) {
    ...
    try {
      ...
      console.log(
        ...
        this.state.callData.incomingcall_id,
      );
      await notifee.cancelNotification(this.state.callData.incomingcall_id);
  ...
  declineCallWebService() {
    ...
    Utils.ApiPost(
      ...
      (response = data => {
        try {
          console.log('call decline response =====================>', data);
          if (data.data.res_status == 'success') {
            console.log(
              'this.state.logTab in response =====>',
              this.state.logTab,
            );
            this.getMessageList(0);
            this.getCallList();
     ...
     <View
           ...
           <TouchableOpacity
             ...
             <Image resizeMode='contain'
               style={{height: 40, width: 40, marginHorizontal: 1,padding:2}}
               source={this.state.selTab == 2 ? R.images.potlis : R.images.potli}
             />
        ...
        <View
          ...
          <TouchableOpacity
            ...
            onPress={() => this.props.navigation.navigate('MostActiveUser')}>
            <Icon name="charity" size={25} />
          </TouchableOpacity>
                ...
                AsyncStorage.getAllKeys()
                  .then(keys => {
                    var keyToRemove = keys.filter(
                      name => name !== 'fcmToken' && name !== 'login_type',
                    );
                  ...
                  .then(async () => {
                    ...
                    this.backHandler.remove();
                    let login_type = await AsyncStorage.getItem('login_type');
                    console.log('in dashboard logout click: ', login_type);
                    this.props.navigation.replace('Landing', {
                      from: 'logOut',
                      type: login_type ? login_type : '',
                    });
                  });
              } catch (exception) {

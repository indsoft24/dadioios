...
import {View,Text,TouchableOpacity,BackHandler,StyleSheet,PermissionsAndroid,Modal} from 'react-native'
...
export default class BasicInfo extends React.Component{
    constructor(props){
        ...
        this.state=({
            ...
            gender:0,
            Name:"",
            age:moment(new Date()).format("DD/MM/YYYY"),
            displayName:'',
            height:"",
            weight:"",
            profession:"",
            ...
            stateList:"",
            cityList:"",
            maxDate:moment().subtract(18, 'year')._d,
            minDate:moment().subtract(70, 'year')._d,
            timer:'',
    ...
    componentDidMount(){
        this.setState({
            basicInfo:this.props.route.params.details
        },()=>{
            this.setState({
                ...
                // age:moment(this.state.basicInfo.age.trim(),"").format("DD/MM/YYYY"),
    ...
    componentWillUnmount() {
      ...
      backAction=()=>{
        this.stopAudio()
        this.props.route.params.backFromBasicInfo();
      }
    ...
    updateBasicInfo(){
        try{
        let url = `${R.constants.Api.updateBasicInfo}${this.state.user_id}${this.state.gender!==0?`&gender=${this.state.gender==1?"Male":this.state.gender==2?"Female":null}`:""}&name=${this.state.Name.trim()}&display_name=${this.state.displayName.trim()}&age=${this.state.age}${this.state.weight!==""?`&weight=${this.state.weight}`:""}${this.state.height!==""?`&height=${this.state.height}`:""}${this.state.profession!==""?`&profession=${this.state.profession.trim()}`:""}${this.state.selectedState!=="0"?`&state=${this.state.selectedState}`:""}${this.state.selectedCity!=="0"?`&city=${this.state.selectedCity}`:""}&use_hand=${this.state.handed==0?"10":"20"}`;
        console.log("==========URL===========",url);
        // return;
                ...
                <KeyboardAwareScrollView style={{flex:1,backgroundColor: "#fff",}}>
                    ...
                    <View style={{flex:1 , padding:20}}>
                        ...
                        {this.state.showPicker&&<DateTimePicker
                            ...
                            testID="datePicker"
                            value={this.convertStringToDate(this.state.age)}
                            mode={'date'}
                            is24Hour={true}
                            display="default"

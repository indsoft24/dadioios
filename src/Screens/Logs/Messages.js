                            ...
                            <View  style={{padding:10,flexDirection:'row',alignItems:'center',flexDirection:'row'}}>
                                <View style={{paddingHorizontal:10,}}>
                                    <TouchableOpacity onPress={()=>{
                                        if(item && item.profile_id && this.props.profileNavigate){
                                    ...
                                    {
                                        item && item.unread_msg && item.unread_msg!=="0" &&
                                ...
                                <TouchableOpacity onPress={()=>{
                                    if(item && this.props.messageClickHere){
                                        this.setState({pageNum:0},()=>this.props.messageClickHere(item));
                                    }
                                    ...
                                    <Text numberOfLines={1} style={{fontSize:12}}>
                                        {item && item.msg_text ? item.msg_text : ''}
                                    </Text>
                                </TouchableOpacity>
                                <Text style={{flex:1,fontSize:12,marginHorizontal:5,color:'grey'}}>
                                    {item && item.msg_time ? item.msg_time : ''}
                                </Text>

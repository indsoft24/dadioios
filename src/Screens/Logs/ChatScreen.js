...
componentDidMount() {
        ...
        Utils.getData("userData", value = (data) => {
            ...
            this.setState({ user_id: userData.user_id, AccountInfo: userData }, () => {
                const data = this.props.route.params.profileId
                this.setState({
                    profile_id: data
                }, () => {
...
refreshData() {
    ...
    backAction = () => {
        console.log("backhandel++++++++", this.props.route.params)
        //   alert(JSON.parse(JSON.stringify(this.props.route.params)))
        try {
            this.props.route.params.backFromChatScreen("messageList", "");
        } catch (e) {
            console.log("error", e);
        }
    ...
    onLineBackAction = () => {
        ...
        this.props.route.params.backFromChatScreen("OnlineUser", "");
    }
            ...
            <Provider>
                <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flexDirection: 'row', elevation: 5, backgroundColor: "#fff", }}>
                        ...
                        <Menu
                            ref={this.setMenuRef}
                            visible={this.state.canShowMenu}
                            onDismiss={()=>{this.setState({canShowMenu:false})}}
                            anchor={
                                <TouchableOpacity
                        ...
                        // }
                        >
                            <Menu.Item onPress={this.option1Click} title={'View Profile'} />
                            {this.state.chatMessages.mylike > 0 && <Menu.Item onPress={this.option2Click} title={'Cancel Crush?'}/>}
                            <Menu.Item onPress={this.option3Click} title={'Clear chat history'}>
                                
                            </Menu.Item>
                            <Menu.Item onPress={this.option4Click} title={this.state.block == "20" ? "Unblock" : "Block"} />
                            <Menu.Item onPress={this.option5Click} title={'Report'}/>
                        </Menu>
                    </View>
                    <View style={{ flex: 1 }}>
                        <View style={{ flex: 1 }}>

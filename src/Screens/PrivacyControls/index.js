...
export default class PrivacyControls extends React.Component{
    ...
    componentDidMount(){
        this.setState({
            call:this.props.route.params.details.random_call=="10"?true:false,
            chat:this.props.route.params.details.random_chat=="10"?true:false,
        })
    }

    updatePrivacy(){
        Utils.ApiPost(`${R.constants.Api.updatePrivacy}${this.props.route.params.details.user_id}&random_call=${this.state.call?"10":"20"}&random_chat=${this.state.chat?"10":"20"}`,response=(data)=>{
            if(data.res==200){
